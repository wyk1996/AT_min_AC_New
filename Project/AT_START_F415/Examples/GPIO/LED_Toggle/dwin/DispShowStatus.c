...
const _SHOW_Reason Showstopreason[STOP_MAX] =
{
	...
	{PE_Cheack_Fail,    "PE故障",   6},
	{Out_short,         "输出短路", 8},
	{Relay_adhesionL,  "火继电器粘",10},
	{Relay_adhesionN,  "零继电器连",10},
};
...
uint8_t show_fail(STOP_REASON Stop_reasonnum)
{
   if(Stop_reasonnum < STOP_MAX)
   {
     PrintStr(0x109B,(uint8_t *)Showstopreason[Stop_reasonnum].pStr,Showstopreason[Stop_reasonnum].ShowLen); 
   }
   else
   {
       PrintStr(0x109B,(uint8_t *)" ",1);  
   }
}
