...
#define YKC_SEND_CMD_93   		0x93			//ftp升级应答
#define YKC_RECV_CMD_94 		0x94			//ftp升级
...
//    uint8_t	Year;
    ...
    uint8_t Day 	: 5;  //一个月的某一天（几号）
    uint8_t wday 	: 3;  //一个周的某一天（星期1-7）
...
__packed typedef struct {
    uint8_t DevNun[7];   	 	//BCD  不足7位补充0
    uint8_t Updatastate;       //升级状态 BIN 码 1
} _YKC_SEND_CMD93;  //升级应答

__packed typedef struct {
    uint8_t  DevNun[7];   	 	  //BCD  不足7位补充0
    uint8_t Pile_model;		      //桩型号 0x01直流   0x02交流
    uint16_t  Pile_KW;	          //桩功率
    uint8_t FTPupdataaddr[16];    //升级服务器地址 ASCII 码 16    不足 16 位补零
    uint16_t FTP_updataPort;      //升级服务器端口 BIN 码 2    不足 2 位补零
    uint8_t FTP_logoinuser[16];    //用户名 ASCII码 16   不足 16 位补零
    uint8_t FTP_logoinpass[16];    //密码 ASCII码 16     不足 16 位补零
    uint8_t FTP_FilePath[32];     //文件路径 ASCII 码 32  不足 32 位补零，文件路径名由平台定义
    uint8_t FTP_control;      //执行控制 BIN 码 1 0x01：立即执行  0x02：空闲执行
    uint8_t FTP_timeout;    //下载超时时间 BIN 码 1 单位：min
} _YKC_RECV_CMD94;
...
_YKC_SEND_CMD93 YKCSENDCMD93;  //ftp远程升级应答
_YKC_RECV_CMD94 YKCRecvCmd94;   //ftp远程升级
...
static uint8_t  YKC_RecvFTPupdata(uint8_t *pdata,uint16_t len);
_4G_RECV_TABLE YKCRecvTable[YKC_RECV_FRAME_LEN] = {
    ...
    {YKC_RECV_CMD_94			,	YKC_RecvFTPupdata		}, 		//远程更新升级
};
...
**Create_Time:
***************************************************************/
uint8_t YKC_SendSetupdataAck(void)
{
    memcpy(YKCSENDCMD93.DevNun,YCKDevNum,sizeof(YCKDevNum));       //装编号
    YKCSENDCMD93.Updatastate = 0x00;   //0x00-成功  0x01-编号错误   0x02-程序与桩型号不符  0x03-下载更新文件超时
    return YKCFreamSend(YKC_SEND_CMD_93,(uint8_t*)&YKCSENDCMD93,sizeof(_YKC_SEND_CMD93));
}
...
/***************************************************************
**Function   :YKC_RecvFTPupdata
**Description://YKC接收远程升级
**Input      :pdata: [输入/出]
**			 len: [输入/出]
**Output     :
**Return     :
...
static uint8_t  YKC_RecvFTPupdata(uint8_t *pdata,uint16_t len)
{
    ...
    memcpy((uint8_t*)&YKCRecvCmd94,pdata,len);

    if(YKCRecvCmd94.Pile_model != 0x02)
    {
        return FALSE;      //桩型号 0x01直流   0x02交流
    }

    YKCRecvCmd94.FTPupdataaddr[15] = 0;    //最后一个肯定为0， 做保护，防止越界
    YKCRecvCmd94.FTP_logoinuser[15] = 0;
    YKCRecvCmd94.FTP_logoinpass[15] = 0;
    YKCRecvCmd94.FTP_FilePath[31] = 0;


    //ftp 服务器IP地址
    length = snprintf(FTPInfo.FTPServerAdd,sizeof(YKCRecvCmd94.FTPupdataaddr),"%s",YKCRecvCmd94.FTPupdataaddr);
    if(length == -1)
    {
        return FALSE;
    }
    ...
    //ftp端口
    FTPInfo.FTPServerport = YKCRecvCmd94.FTP_updataPort;

    //ftp用户名
    length = snprintf(FTPInfo.FTPServeruser,sizeof(YKCRecvCmd94.FTP_logoinuser),"%s",YKCRecvCmd94.FTP_logoinuser);
    if(length == -1)
    {
        return FALSE;
    }
    //ftp密码
    length = snprintf(FTPInfo.FTPServerpass,sizeof(YKCRecvCmd94.FTP_logoinpass),"%s",YKCRecvCmd94.FTP_logoinpass);
    if(length == -1)
    {
        return FALSE;
    }
    //ftp文件路径
    length = snprintf(FTPInfo.FTPServer_Path,sizeof(YKCRecvCmd94.FTP_FilePath),"%s",YKCRecvCmd94.FTP_FilePath);
    if(length == -1)
    {
        return FALSE;
    }

    FTPInfo.FTPupadatflag = 1;   //是否升级的标志位
    mq_service_send_to_4gsend(FTP_UPDADA_ACK,0,0,NULL);  //升级应答成功
    return TRUE;
}
...
uint8_t  APP_YKCSENDFEILV(uint32_t timeout)
{
    ...
    {
        checkYKConline = 1;
        current_time = OSTimeGet(&timeerr);   //清空时间
    ...
    if(APP_GetAppRegisterState(LINK_NUM) == STATE_OK)  //注册成功后 联网的状态下
    {
        ...
        {
            current_time = OSTimeGet(&timeerr);   //正常返回标志位和时间清空
            last_time = OSTimeGet(&timeerr);
            checkYKConline = 1;
        }
    }
