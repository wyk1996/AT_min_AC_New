...
#if(UPDATA_STATE)
uint8_t DataBuf[URART_4GRECV_LEN];	//接收数据缓存
#else
uint8_t DataBuf[250];	//接收数据缓存
#endif
    uint16_t len;			//接收数据长度
...
typedef struct
{
    char FTPServerAdd[16];
	uint16_t FTPServerport; //升级服务器端口
	char FTPServeruser[16]; //用户名
	char FTPServerpass[16]; //密码
	char FTPServer_Path[32]; //文件路径
	uint32_t FTPupadatSize; //下载文件的大小
	
	uint8_t FTPDownloadflag; //开始下载标志位 （在开始写入时用到）
	uint8_t FTPDownloadNUM; //下载次数
    uint8_t FTPDownloadUP; //下载完成标志位
	
	uint8_t FTPupadatflag; //接收服务器是否下发升级标志位
}_FTP_INFO;
...
#endif
extern uint8_t SIM7600Reset123();
