...
#endif
#if(FTPUPDATA_FLAG)
    uint8_t FTP_StartRecv;							//ftp开始返回
    uint8_t FTP_logonRecv;							//ftp登录返回
    uint32_t FTP_SizeRecv;							//ftp文件大小
    uint8_t FTP_TypeRecv;							//ftp文件类型
    uint8_t FTP_dwonloadRecv;						//ftp下载完成
#endif
...
uint8_t Send_AT_startftpACK(uint8_t *pdata, uint16_t len)
{
    if((pdata == NULL) || !len)
    {
        return FALSE;
    ...
    pdata[len] = '\0';  //结束符
    if (strstr((char *)pdata, "+CFTPSSTART: 0") == NULL)
    {
        return FALSE;
    }
    OSSchedLock(&ERRSTATE);
    SIM7600CmdState.FTP_StartRecv = _4G_RESPOND_OK;
...
**Create_Time:
***************************************************************/
uint8_t Send_AT_ftplogin(char *serveraddr,uint16_t port,char *user,char *pass)
{
	uint8_t ftpbuf[100]={0};  //ftpbuf发送的buff
	memset(ftpbuf,0,sizeof(ftpbuf)); //清空一下
    int length;
    length = snprintf((char *)ftpbuf, sizeof(ftpbuf), "AT+CFTPSLOGIN=\"%s\",%d,\"%s\",\"%s\",0\r\n",serveraddr,port,user,pass);
    if (length == -1)
    {
...
uint8_t Send_AT_ftploginACK(uint8_t *pdata, uint16_t len)
{
    if((pdata == NULL) || !len)
    {
        return FALSE;
    ...
    pdata[len] = '\0';  //结束符
    OSSchedLock(&ERRSTATE);
    SIM7600CmdState.FTP_logonRecv = _4G_RESPOND_OK;
    OSSchedUnlock(&ERRSTATE);
    return TRUE;
}
...
/***************************************************************
**Function   :Send_AT_ftpdwonloadtype
**Description:ftp下载文件设置传出类型  AT+CFTPSTYPE=A  A=ASCII    I=Binary (二进制)
**Input      :None
**Output     :
**Return     :
**note(s)    :
...
**Create_Time:
***************************************************************/
uint8_t Send_AT_ftpdwonloadtype(void)
{
    if(UART_4GWrite((uint8_t*)"AT+CFTPSTYPE=I\r\n",strlen("AT+CFTPSTYPE=I\r\n")) == strlen("AT+CFTPSTYPE=I\r\n"))
    {
        return TRUE;
    }
    return FALSE;
...
uint8_t Send_AT_ftpdwonloadtypeACK(uint8_t *pdata, uint16_t len)
{
    if((pdata == NULL) || !len)
    {
        return FALSE;
    ...
    pdata[len] = '\0';  //结束符
	if (strstr((char *)pdata, "+CFTPSTYPE: 0") == NULL)
    {
        return FALSE;
    }
	
    OSSchedLock(&ERRSTATE);
    SIM7600CmdState.FTP_TypeRecv = _4G_RESPOND_OK;
    OSSchedUnlock(&ERRSTATE);
    return TRUE;
}


////========ftp登录成功后， 进入所在的目录中所有项目 读出二级列表==============
//uint8_t Send_AT_ftptwolist(void)
//{
//    if(UART_4GWrite((uint8_t*)"AT+CFTPSPWD=\"/cehi\"\r\n",strlen("AT+CFTPSPWD=\"/cehi\"\r\n")) == strlen("AT+CFTPSPWD=\"/cehi\"\r\n"))
//    {
//        return TRUE;
//    }
//    return FALSE;
...
/***************************************************************
**Function   :Send_AT_ftpdwonloadsize
**Description:获取ftp文件大小
**Input      :None
**Output     :
**Return     :
**note(s)    :
...
**Create_Time:
***************************************************************/
uint8_t Send_AT_ftpdwonloadsize(char *name)
{
	uint8_t ftpbuf[50];  //ftpbuf发送的buff
    int length;
    length = snprintf((char *)ftpbuf, sizeof(ftpbuf),"AT+CFTPSSIZE=\"%s\"\r\n",name);
    if (length == -1)
    {
        printf("snprintf error, the len is %d", length);
        return FALSE;
...
uint8_t Send_AT_ftpdwonloadsizeACK(uint8_t *pdata, uint16_t len)
{
    if((pdata == NULL) || !len)
    {
        return FALSE;
	...
	pdata[len] = '\0';  //结束符
    FTPInfo.FTPupadatSize = atoi((const char *)&pdata[strlen("\r\nOK\r\n\r\n+CFTPSSIZE: ")]);
    if(FTPInfo.FTPupadatSize == 0)
    {
        return FALSE;
    }
    OSSchedLock(&ERRSTATE);
    SIM7600CmdState.FTP_SizeRecv = _4G_RESPOND_OK;
    OSSchedUnlock(&ERRSTATE);
    return TRUE;
...
/***************************************************************
**Function   :Send_AT_ftpdwonload
**Description:ftp下载文件
**Input      :data: [输入/出]   传入下载的文件名
**Output     :
**Return     :
**note(s)    :
**Author     :CSH
...
***************************************************************/
uint8_t Send_AT_ftpdwonload(char *name)
{
	uint8_t ftpbuf[50];  //ftpbuf发送的buff
    int length;
...
/***************************************************************
**Function   :Send_AT_ftploginout
**Description://关闭ftp登录服务器
**Input      :None
**Output     :
**Return     :
**note(s)    :
...
**Create_Time:
***************************************************************/
uint8_t Send_AT_ftploginout(void)
{
    if(UART_4GWrite((uint8_t*)"AT+CFTPSLOGOUT \r\n",strlen("AT+CFTPSLOGOUT \r\n")) == strlen("AT+CFTPSLOGOUT \r\n"))
    {
        return TRUE;
    }
    return FALSE;
...
//    UART_4GWrite((uint8_t*)"AT+CGDCONT=1,\"IP\",\"cmnet\"\r\n", strlen("AT+CGDCONT=1,\"IP\",\"cmnet\"\r\n") );
//    //Send_AT_CGDCONT();
//    //等待2s回复
//    i = 0;
...
//    if(i == CM_TIME_2_SEC / CM_TIME_100_MSEC)
//    {
//        printf("respond cmd:NOTCIPMODE_timeout");   //回复超时
//        return FALSE;
//    }
    ...
    {
        OSTimeDly(CM_TIME_100_MSEC, OS_OPT_TIME_PERIODIC, &timeerr);
        if(SIM7600CmdState.FTP_StartRecv == _4G_RESPOND_OK)
        {
            SIM7600CmdState.FTP_StartRecv = _4G_RESPOND_ERROR;
            break;
        }
        i++;
    }
    if(i == CM_TIME_2_SEC / CM_TIME_100_MSEC)
    {
        printf("respond cmd:startftp_timeout");   //回复超时
        return FALSE;
    }


    //用AT指令开始登录ftp +等待回复
    Send_AT_ftplogin(FTPInfo.FTPServerAdd,FTPInfo.FTPServerport,FTPInfo.FTPServeruser,FTPInfo.FTPServerpass);
    OSTimeDly(CM_TIME_500_MSEC, OS_OPT_TIME_PERIODIC, &timeerr);
    i = 0;
    while (i < CM_TIME_10_SEC / CM_TIME_100_MSEC)
    {
        OSTimeDly(CM_TIME_100_MSEC, OS_OPT_TIME_PERIODIC, &timeerr);
        if(SIM7600CmdState.FTP_logonRecv == _4G_RESPOND_OK)
        {
            SIM7600CmdState.FTP_logonRecv = _4G_RESPOND_ERROR;
            break;
        }
        i++;
    }
    if(i == CM_TIME_10_SEC / CM_TIME_100_MSEC)
    {
        printf("respond cmd:ftplogin_timeout");   //回复超时
        return FALSE;
    }

	
    //==设置ftp下载的文件类型+ 等待回复
    Send_AT_ftpdwonloadtype();
    OSTimeDly(CM_TIME_500_MSEC, OS_OPT_TIME_PERIODIC, &timeerr);
    i = 0;
    while (i < CM_TIME_2_SEC / CM_TIME_100_MSEC)
    {
        OSTimeDly(CM_TIME_100_MSEC, OS_OPT_TIME_PERIODIC, &timeerr);
        if(SIM7600CmdState.FTP_TypeRecv == _4G_RESPOND_OK)
        {
            SIM7600CmdState.FTP_TypeRecv = _4G_RESPOND_ERROR;
            break;
        }
        i++;
    }
    if(i == CM_TIME_2_SEC / CM_TIME_100_MSEC)
    {
        printf("respond cmd:FTP_ftpdwonloadtype_timeout");   //回复超时
        return FALSE;
    }

    //==获取ftp下载大小 + 等待回
    Send_AT_ftpdwonloadsize(FTPInfo.FTPServer_Path);
    OSTimeDly(CM_TIME_500_MSEC, OS_OPT_TIME_PERIODIC, &timeerr);
    i = 0;
    while (i < CM_TIME_2_SEC / CM_TIME_100_MSEC)
    {
        OSTimeDly(CM_TIME_100_MSEC, OS_OPT_TIME_PERIODIC, &timeerr);
        if(SIM7600CmdState.FTP_SizeRecv == _4G_RESPOND_OK)
        {
            SIM7600CmdState.FTP_SizeRecv = _4G_RESPOND_ERROR;
            break;
        }
        i++;
    }
    if(i == CM_TIME_2_SEC / CM_TIME_100_MSEC)
    {
        printf("respond cmd:FTP_ftpdwonloadsize_timeout");   //回复超时
        return FALSE;
    }

    Send_AT_ftpdwonload(FTPInfo.FTPServer_Path);      //开始正式下载文件
	FTPInfo.FTPDownloadflag = 1;
    return TRUE;
}
#endif
...
#warning "存储   YXY 20220531"
        ...
        if((pcontrol->AllLen - down_len) > 300)
        {
            Send_AT_READ(300);   //一次一次读取
        }
        else
        {
...
#if(FTPUPDATA_FLAG)
    //FTP
    {"AT+CFTPSSTART"				,		Send_AT_startftpACK						},		//ftp开始

    {"\r\n+CFTPSLOGIN: 0"			,		Send_AT_ftploginACK						},		//ftp登录成功返回

    {"AT+CFTPSTYPE"				   ,		Send_AT_ftpdwonloadtypeACK				},		//ftp数据类型

    {"AT+CFTPSSIZE:"			   ,		Send_AT_ftpdwonloadsizeACK			    },		//ftp数据大小

    {"\r\nOK\r\n\r\n+CFTPSSIZE"		,		Send_AT_ftpdwonloadsizeACK				},		//真实的ftp数据大小
#endif
};
