...
uint8_t PE_CHEACK6[40];
uint8_t PE_CHEACK7[40];
uint8_t ZHANlian_JC[40];  //L和N继电器是否粘连检测
uint8_t Short_JC[40];  //LN输出短路检测
...
{   static uint8_t count = 0;
    if(TMR4->STS&0X0001)//溢出中断
    {
		...
		static uint8_t countNUM1 = 0,countNUM2 = 0;
		if(countNUM1++>50)
		{
			countNUM1 = 0;
			PE_CHEACK6[countNUM2] = GPIO_ReadInputDataBit(GPIOF,GPIO_Pins_6);
			PE_CHEACK7[countNUM2] = GPIO_ReadInputDataBit(GPIOF,GPIO_Pins_7);
			ZHANlian_JC[countNUM2] = GPIO_ReadInputDataBit(GPIOC,GPIO_Pins_5);
			Short_JC[countNUM2] = GPIO_ReadInputDataBit(GPIOC,GPIO_Pins_0);
			countNUM2++;
			if(countNUM2 >= 40)
			{
			  countNUM2 = 0;
